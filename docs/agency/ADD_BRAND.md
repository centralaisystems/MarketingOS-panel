...
It reads the registry for the brand switcher, then loads **only** the active `brand_id` for readiness, drafts, inbox, audit, asset metadata, Drive sync status, analytics snapshots, and AI search visibility. Cross-brand query/body mismatches return `403 CROSS_BRAND_DENIED`; another brand's campaign/asset id returns `404`.
