...
Villa Glory is first (fixture `owner_email` on the registry). LOTIN, NOX FORM, and NOX TECH stay off in the committed registry so dry-run mail cannot target a real inbox — opt in with a gitignored `REGISTRY.local.json` overlay using `@example.test` (see [`ADD_BRAND.md`](./ADD_BRAND.md)). Other brands opt in per entry.
