...
Copy pulls pack/approval/digest fields only. It never invents `VERIFIED` commercial claims and does not attach binaries. When Figma arrange or Higgsfield fill-gap jobs exist for the campaign, the review payload includes `arranged_asset_ids` / `figma_job_ids` and `generated_asset_ids` / `higgsfield_job_ids` (GENERATED / UNVERIFIED pointers) so the owner page can deep-link later. Existing approve / request-changes behavior is unchanged.
