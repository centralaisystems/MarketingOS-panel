/**
 * NOX TECH golden path (fixture mode) — thinner than Villa Glory.
 * Drive sync → campaign pack → digest dry-run (test overlay) → isolation vs VG / NOX FORM.
 * Committed NOX TECH registry keeps automation/owner email off.
 * Asserts live publish/ads stay off. Does not reintroduce rejected packages/ROI.
 */
...
import {
  ...
  NOX_TECH_FIXTURE_OWNER_CC,
  NOX_TECH_FIXTURE_OWNER_EMAIL,
  WAVE4_ASSET_IDS,
...
function writeNoxTechAutomationOverlay(root: string): void {
  writeFileSync(
    ...
    JSON.stringify({
      brands: [
        {
          brand_id: "NOX_TECH",
          owner_email: NOX_TECH_FIXTURE_OWNER_EMAIL,
          owner_cc: [NOX_TECH_FIXTURE_OWNER_CC],
...
function ctx(opts?: { overlayAutomation?: boolean }): PanelApiContext & {
  ...
  const reportRoot = mkdtempSync(join(tmpdir(), "mos-nt-golden-"));
  const brandsRoot = opts?.overlayAutomation
    ? mkdtempSync(join(tmpdir(), "mos-nt-reg-"))
    : undefined;
  if (brandsRoot) {
    cpSync(REPO_BRANDS, brandsRoot, { recursive: true });
    writeNoxTechAutomationOverlay(brandsRoot);
  }
  return {
...
describe("NOX TECH golden path (fixture)", () => {
  const temps: string[] = [];
  afterEach(() => {
    clearBrandRegistryCache();
  ...
  it("runs Drive → pack → digest overlay without live flags or foreign-brand leak", async () => {
    ...
    const committed = getBrandEntry("NOX_TECH");
    expect(committed.asset_drive_folder_id).toBe(NOX_TECH_FIXTURE_DRIVE_FOLDER_ID);
    ...
    const blocked = await api(blockedCtx, "POST", "/api/digests", {
      body: { brand_id: "NOX_TECH", period: "daily" },
    });
    ...
    ).brands.find((row) => row.brand_id === "NOX_TECH");
    expect(ntEntry?.asset_drive_folder_id).toBe(NOX_TECH_FIXTURE_DRIVE_FOLDER_ID);
    expect(ntEntry?.automation_enabled).toBe(true);
    expect(ntEntry?.owner_email).toBe(NOX_TECH_FIXTURE_OWNER_EMAIL);

    const ready = await api(c, "GET", "/api/readiness", {
      query: { brand_id: "NOX_TECH" },
    });
    ...
    expect(readyBody.brand_id).toBe("NOX_TECH");
    ...
    expect(seededRows.some((row) => row.asset_id === WAVE4_ASSET_IDS.NOX_TECH_UNUSED)).toBe(
      true,
    );
    expect(
    ...
    expect(
      seededRows.some((row) => row.asset_id === WAVE4_ASSET_IDS.NOX_FORM_UNUSED),
    ).toBe(false);

    const synced = await api(c, "POST", "/api/drive-sync", {
      body: { brand_id: "NOX_TECH" },
    });
    ...
    expect(syncBody.brand_id).toBe("NOX_TECH");
    ...
    const nfDrive = await api(c, "GET", "/api/assets", {
      query: { brand_id: "NOX_FORM", source: "drive" },
    });
    expect(nfDrive.status).toBe(200);
    expect((nfDrive.body as { assets: unknown[] }).assets).toEqual([]);

    const ntAssets = await api(c, "GET", "/api/assets", {
      query: { brand_id: "NOX_TECH", source: "drive" },
    });
    expect(ntAssets.status).toBe(200);
    const ntAssetRows = (
      ntAssets.body as { assets: Array<{ asset_id: string; brand_id: string }> }
    ).assets;
    expect(ntAssetRows.length).toBeGreaterThan(0);
    expect(ntAssetRows.every((row) => row.brand_id === "NOX_TECH")).toBe(true);
    ...
    expect(vgRows.some((row) => ntAssetRows.some((n) => n.asset_id === row.asset_id))).toBe(
      false,
    );
    ...
    const packRes = await api(c, "POST", "/api/campaign-packs", {
      body: {
        brand_id: "NOX_TECH",
        objective: "Draft a qualified discovery plan for B2B implementation conversations",
      },
    });
    ...
    expect(pack.brand_id).toBe("NOX_TECH");
    ...
    expect(digestBody.digest.brand_id).toBe("NOX_TECH");
